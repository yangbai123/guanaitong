(function(root, factory) {
    ...
    var defaults = {
        width: 280,     // 宽度
        height: 280,    // 高度, 不包含头部，头部固定高度
        zIndex: 1,    
        trigger: null,      // selector or element   设置触发显示的元素，为null时默认显示
        offset: [0, 1],    // 偏移位置，可设正负值    trigger 设置时生效
        customClass: '',    // 自定义类，用于重写样式
        view: 'date',       // 显示视图     可选：date, month
        date: new Date(),   // 默认日期为当前日期
        format: 'yyyy/mm/dd', 

        startWeek: 0,    // 一周的第一天    0表示周日，依次类推
        weekArray: ['日', '一', '二', '三', '四', '五', '六'],    // 星期格式
    ...
    Date.prototype.formats = function(exp) {
    ...
    Calendar.prototype = {
        ...
        hide: function(view, date, data) {
            this.$trigger.val(date.formats(this.options.format));
        ...
        trigger: function() {
            ...
            $this.addClass('calendar-modal').css({
                left: '0px',// ||(post.left + offs[0]) + 'px',
                top: '-13px',// ||(post.top + _this.$trigger.outerHeight() + offs[1]) + 'px',
                zIndex: _this.options.zIndex
            });
            //console.log(_this.options.position.x,_this.options.position.y,(post.left + offs[0]))
        ...
        showLabel: function(event, view, date, data) {
            ...
            $lbl.find('p').html(this.options.label.repeat({
                m: view,
                d: date.formats(this.options.format),
                v: data
            }).replace(/\n/g, '<br>'));
